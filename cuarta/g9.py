...
def cramer_von_mises(data):
    ...
    def cvm_integrand(x):
        return (ecdf(x) - norm.cdf(x, 0, 2.5))**2
    data[-1] = np.inf
    data[len(data)] = np.inf
    jumps = sorted(data.unique())
    cvm = 0
    cvm = integrate.quad(cvm_integrand, -np.inf, min(data))[0]
    cvm += integrate.quad(fun, max(data), np.inf)[0]
    for a, b in zip(jumps[:-1], jumps[1:]):
        cvm += integrate.quad(cvm_integrand, a, b)[0]
    return cvm
...
def plot_hist(data, nbins=56):
    ax = sns.distplot(data, nbins, kde=False,
                      hist_kws={'range': (-7, 7), 'label': 'Datos'})
...
def main(args):
    ...
    if '14a' in args.items:
        x = np.array([2.00, 2.10, 2.20, 2.30, 2.40, 2.50, 2.60, 2.70, 2.80,
                      2.90, 3.00])
        y = np.array([2.78, 3.29, 3.29, 3.33, 3.23, 3.69, 3.46, 3.87, 3.62,
                      3.40, 3.99])
        s = 0.3
        ej14(x, y, s)
